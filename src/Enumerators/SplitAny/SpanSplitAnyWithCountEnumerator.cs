...
{
    /// <summary>
    /// Supports iteration over a <see cref="Span{T}"/> by splitting it at specified delimiters of type <typeparamref name="T"/> with an upper limit of splits performed.
    /// </summary>
    ...
    public ref struct SpanSplitAnyWithCountEnumerator<T> where T : IEquatable<T>
    {
        ...
        /// <summary>
        /// Constructs a <see cref="SpanSplitAnyEnumerator{T}"/> from a span and a delimiter. <strong>Only consume this class through <see cref="SpanExtensions.SplitAny{T}(Span{T}, ReadOnlySpan{T}, int, CountExceedingBehaviour)"/></strong>.
        /// </summary>
        ...
        public SpanSplitAnyWithCountEnumerator(Span<T> source, ReadOnlySpan<T> delimiters, int count, CountExceedingBehaviour countExceedingBehaviour = CountExceedingBehaviour.AppendRemainingElements)
        {
            Span = source;
